...
- **Database**: Cloud SQL `YOUR_GCP_PROJECT_ID:us-central1:users`
...
~/bin/cloud-sql-proxy --port 5432 YOUR_GCP_PROJECT_ID:us-central1:users &
...
ENV_FILE=.env.cloudrun INSTANCE_CONNECTION="YOUR_GCP_PROJECT_ID:us-central1:users" \
   ./scripts/sync-env.sh
```
...
# Or manually
docker build -t gcr.io/YOUR_GCP_PROJECT_ID/humanchat-api:latest .
docker push gcr.io/YOUR_GCP_PROJECT_ID/humanchat-api:latest

gcloud run deploy humanchat-api \
  --image=gcr.io/YOUR_GCP_PROJECT_ID/humanchat-api:latest \
  --platform=managed \
  --region=us-central1 \
...
# Navigate to: Cloud SQL → YOUR_GCP_PROJECT_ID → Monitoring
```
...
gcloud sql instances describe users --project=YOUR_GCP_PROJECT_ID
